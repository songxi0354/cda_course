...
def main():
    ...
    try:
        ...
        while stream.is_active():
            ...
            # 检查超时逻辑（自动回说话场景）
            if current_scene != SCENE_TALK and (time.time() - last_action_time) > SILENCE_TIMEOUT:
            ...
            if text:
                ...
                found_welcome = any(word in text for word in KEYWORDS_WELCOME)
                if found_talk:
                    ...
                    last_action_time = time.time()
                    if current_scene != SCENE_WELCOME:
                        print(f"🚀 识别到关键词 -> 切换至: {SCENE_WELCOME}")
                        client.set_current_program_scene(SCENE_WELCOME)
                        current_scene = SCENE_WELCOME
                        rec.Reset()
